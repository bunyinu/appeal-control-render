...
export const getButtonColor = (
  color: ColorButtonKey | string,
  isOutlined: boolean,
  hasHover: boolean,
  isActive = false
) => {
  const fallbackColor = (
    ['white', 'whiteDark', 'lightDark', 'contrast', 'success', 'danger', 'warning', 'info', 'void'].includes(color)
      ? color
      : 'white'
  ) as ColorButtonKey

  if (fallbackColor === 'void') {
    return ''
  }
  ...
  const isOutlinedProcessed = isOutlined && ['white', 'whiteDark', 'lightDark'].indexOf(fallbackColor) < 0

  const base = [colors.borders[fallbackColor], colors.ring[fallbackColor]]

  if (isActive) {
    base.push(colors.active[fallbackColor])
  } else {
    base.push(isOutlinedProcessed ? colors.text[fallbackColor] : colors.bg[fallbackColor])
  }

  if (hasHover) {
    base.push(isOutlinedProcessed ? colors.outlineHover[fallbackColor] : colors.bgHover[fallbackColor])
  }
