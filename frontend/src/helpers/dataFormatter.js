...
export default {
    ...
    dataGridEditFormatter(obj) {
        ...
        payersManyListFormatter(val) {
            if (!val || !val.length) return []
            return val.map((item) => item.name)
        },
        payersOneListFormatter(val) {
            if (!val) return ''
            return val.name
        },
        payersManyListFormatterEdit(val) {
            if (!val || !val.length) return []
            return val.map((item) => {
              return {id: item.id, label: item.name}
            });
        },
        payersOneListFormatterEdit(val) {
            if (!val) return ''
            return {label: val.name, id: val.id}
        },
        ...
        casesManyListFormatter(val) {
            if (!val || !val.length) return []
            return val.map((item) => item.case_number)
        },
        casesOneListFormatter(val) {
            if (!val) return ''
            return val.case_number
        },
        casesManyListFormatterEdit(val) {
            if (!val || !val.length) return []
            return val.map((item) => {
              return {id: item.id, label: item.case_number}
            });
        },
        casesOneListFormatterEdit(val) {
            if (!val) return ''
            return {label: val.case_number, id: val.id}
        },
